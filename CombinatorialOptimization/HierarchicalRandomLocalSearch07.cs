...
namespace CombinatorialOptimization
{
    ...
    class HierarchicalRandomLocalSearch07 : IOptimizationAlgorithm
    {
        private long limit;
        private int trj;
        ...
        public ISolution Solve(IOptimizationProblem p, ISolution sol, DataStoringWriter w)
        {
            ...
            for (int loop = 1; true; loop++)
            {
                ...
                while (High(root).Count() * 3 < Mid(root).Count())
                {
                ...
                //Low探索
                if (0 < Low(root).Count() && Low(root).Min((n) => n.s.Value) == Low(root).Min((n) => n.best.Value))
                {
        ...
        public HierarchicalRandomLocalSearch07(long limit, int trj, double targetHighAct, double targetMidAct, double targetLowAct, int range)
        {
            this.limit = limit;
            this.trj = trj;
