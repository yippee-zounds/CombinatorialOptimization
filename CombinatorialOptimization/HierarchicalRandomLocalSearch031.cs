...
{
    class HierarchicalRandomLocalSearch031 : IOptimizationAlgorithm
    {
        private long limit;
        private int trj;
        ...
        public ISolution Solve(IOptimizationProblem p, ISolution sol, DataStoringWriter w)
        {
            ...
            for (int loop = 1; true; loop++, bestLoop++, subLoop1++, subLoop2++)
            {
                ...
                Console.WriteLine(loop + ":" + ret.Value + ":" + bestDom.Value + ":" + All(root).Min((n) => n.s.Value)
                    ...
                    + ":" + Con(root).Count() + ":" + Low(root).Count() + ":" + Mid(root).Count() + ":" + endCount + ":" + mode);

                //プールの構成
                //////Midレベル
                if (1000 < root.loop && (root.child.Count() == 0 || root.rangeBestLoop == 0))
                {
                    root.pool.Add(new HierarchicalRandomLocalSearchElement(root, root.best, targetMidAct, midIdCount++));
                }

                foreach (var e in Low(root).Where((n) => 10 < n.bestLoop))
                {
                    root.pool.Add(new HierarchicalRandomLocalSearchElement(e, targetMidAct, midIdCount++));
                    e.removeFlag = true;
                }

                foreach (var ms in Mid(root).Select((n) => n.pool))
                {
                    var tmp = new List<HierarchicalRandomLocalSearchElement>();

                    foreach (var e in ms.Element().Select((n) => (HierarchicalRandomLocalSearchElement)n))
                    {
                        if (e.removeFlag) tmp.Add(e);
                    }
                    ...
                    while(10 < ms.Element().Count())
                    {
                        var rm = ms.Element().Select((n) => (HierarchicalRandomLocalSearchElement)n).ArgMax((n) => n.s.Value);
                        ms.Remove(rm);
                    }
                }

                //////Lowレベル
                foreach (var e in Mid(root).Where((n) => 10 < n.loop && n.rangeBestLoop == 0))
                {
                    e.pool.Add(new HierarchicalRandomLocalSearchElement(e, targetLowAct, lowIdCount++));
                }
                
                foreach(var e in Con(root).Where((n) => 1 < n.bestLoop))
                {
                    Parent(root, Parent(root, e)).pool.Add(new HierarchicalRandomLocalSearchElement(e, targetLowAct, lowIdCount++));
                    e.removeFlag = true;
                }

                foreach (var ms in Low(root).Select((n) => n.pool))
                {
                    var tmp = new List<HierarchicalRandomLocalSearchElement>();
                ...
                //////Conレベル
                foreach (var ms in Con(root).Select((n) => n.pool))
                {
                    var tmp = new List<HierarchicalRandomLocalSearchElement>();
        ...
        */
        ...
        public HierarchicalRandomLocalSearch031(long limit, int trj, double targetHighAct, double targetMidAct, double targetLowAct, int range)
        {
            this.limit = limit;
            this.trj = trj;
