...
{
    class HierarchicalRandomLocalSearch032 : IOptimizationAlgorithm
    {
        private long limit;
        private int trj;
        ...
        public ISolution Solve(IOptimizationProblem p, ISolution sol, DataStoringWriter w)
        {
            ...
            for (int loop = 1; true; loop++, bestLoop++, subLoop1++, subLoop2++)
            {
                ...
                //////Midレベル
                root.child.Clear();
                foreach(var e in root.pool.SelectDiversity(trj * 2).OrderBy((n) => ((HierarchicalRandomLocalSearchElement)n).s.Value).Take(trj))
                {
                ...
                {
                    m.child.Clear();
                    foreach(var e in m.pool.SelectDiversity(trj * 2).OrderBy((n) => ((HierarchicalRandomLocalSearchElement)n).s.Value).Take(trj))
                    {
        ...
        */
        ...
        public HierarchicalRandomLocalSearch032(long limit, int trj, double targetHighAct, double targetMidAct, double targetLowAct, int range)
        {
            this.limit = limit;
            this.trj = trj;
