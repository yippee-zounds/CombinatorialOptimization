...
{
    class HierarchicalComposeSearch02 : IOptimizationAlgorithm
    {
        private long limit;
        private int ctrj;
        private int dtrj;
        ...
        public ISolution Solve(IOptimizationProblem p, ISolution sol, DataStoringWriter w)
        {
            ...
            for (int loop = 0; true; loop++, bestLoop++)
            {
                ...
                //////Lowレベル→Conレベル
                /*for (int i = 0; i < lowCmp.Count(); i++)
                {
                    var x = new HierarchicalRandomLocalSearchElement(p, lowCmp[i].best, targetConAct, p.OperationSet().Count(), range);
                    conSs.Add(x);
                    conCmp.Add(x);
                    x.checkedFlag = true;
                }*/
        ...
        public HierarchicalComposeSearch02(long limit, int ctrj, int dtrj, double targetHighAct, double targetMidAct, double targetLowAct, int range)
        {
            this.limit = limit;
            this.ctrj = ctrj;
            this.dtrj = dtrj;
