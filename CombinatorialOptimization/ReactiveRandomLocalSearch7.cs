...
{
    // RRLS6のTargetActivityとしてTargetMidActを追加
    class ReactiveRandomLocalSearch7 : IOptimizationAlgorithm
    {
        ...
        public ISolution Solve(IOptimizationProblem p, ISolution sol, DataStoringWriter w)
        {
            ...
            for (int loop = 0; true; loop++)
            {
                ...
                Console.WriteLine(loop + ":" + ret.Value + ":" + s.Value + ":" + dom.Value + ":" + s.DistanceTo(p.Optimum).ToString("D4") + ":" + ":" + sizeOfSubset.ToString("D4") + ":" + dm100.DiameterValue + ":" + targetAct.ToString("F2") + ":" + r10.ToString("F2") + ":" + endCount);

                if ( bestLoop < 100)
                {
                    targetAct = targetLowAct;
                }
                else if( bestLoop < targetRange)
                {
                    targetAct = targetMidAct;
                }
                else
                {
        ...
        {
            return this.GetType().Name + "[lim=" + limit + ",hact=" + targetHighAct + ",mact=" + targetMidAct + ",lact=" + targetLowAct + ",trange=" + targetRange + "]";
        }

        public ReactiveRandomLocalSearch7(long limit, double targetHighAct, double targetMidAct, double targetLowAct, int targetRange)
        {
            this.limit = limit;
