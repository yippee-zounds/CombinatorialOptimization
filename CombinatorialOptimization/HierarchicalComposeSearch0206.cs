...
{
    class HierarchicalComposeSearch0206 : IOptimizationAlgorithm
    {
        private long limit;
        private int ctrj;
        private int dtrj;
        ...
        public ISolution Solve(IOptimizationProblem p, ISolution sol, DataStoringWriter w)
        {
            ...
            for (int loop = 0; true; loop++, bestLoop++)
            {
                ...
                //冗長な要素の削除
                Shrink(highSs, highSetSize);
                Shrink(midSs, midSetSize);
                Shrink(lowSs, lowSetSize);
        ...
        public void Shrink(List<HierarchicalRandomLocalSearchElement> ss, int targetSize)
        {
            ...
            while (targetSize < ss.Count())
            {
                ...
                for (int i = 0; i < ss.Count(); i++)
                {
                    ...
                    double v = new MetricSpace(tmp).Variance();
                    if (maxVariance < v)
                    {
                        maxVariance = v;
                        index = i;
            ...
            while (targetSize < ss.Count())
            {
                ...
                for (int i = 0; i < ss.Count() - 1; i++)
                {
                    ...
                    {
                        if (ss[i].DistanceTo(ss[j]) == 0)
                        {
                            if (ss[i].s.Value < ss[j].s.Value) index = j;
                            else index = i;
                        }
                    }
                }
        ...
        public HierarchicalComposeSearch0206(long limit, int ctrj, int dtrj, double targetHighAct, double targetMidAct, double targetLowAct, int range)
        {
            this.limit = limit;
            this.ctrj = ctrj;
            this.dtrj = dtrj;
