...
namespace CombinatorialOptimization
{
    ...
    class ReactiveRandomLocalSearch631 : IOptimizationAlgorithm
    {
        ...
        public ISolution Solve(IOptimizationProblem p, ISolution sol, DataStoringWriter w)
        {
            ...
            for (int loop = 0; true; loop++)
            {
                ...
                Console.WriteLine(loop + ":" + ret.Value + ":" + s.Value + ":" + dom.Value + ":" + s.DistanceTo(p.Optimum).ToString("D4") + ":" + s.DistanceTo(ret).ToString("D4") + ":" + sizeOfSubset.ToString("D4") + ":" + Diameter.CalculateDiameter(ss).DiameterValue + ":" + targetAct.ToString("F2") + ":" + currentAct.ToString("F2") + ":" + endCount + ":" + betterLoop);
                ...
                {
                    if (bestLoop < targetRange)
                    {
                        targetAct = Math.Min(targetAct + 0.1, targetLowAct);
                    }
                    else
                    {
                ...
                else if (1000 < loop && betterLoop == 0)
                {
                    targetAct = Math.Max(targetAct - 0.1, 0);
                }
        ...
        public ReactiveRandomLocalSearch631(long limit, double targetHighAct, double targetLowAct, int targetRange)
        {
            this.limit = limit;
