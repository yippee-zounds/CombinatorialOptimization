...
{
    // RRLS3を性能が大きく落ちない範囲でできるだけシンプルにしたもの
    class ReactiveRandomLocalSearch4 : IOptimizationAlgorithm
    {
        ...
        public ISolution Solve(IOptimizationProblem p, ISolution sol, DataStoringWriter w)
        {
            ...
            for (int loop = 0; true; loop++)
            {
                ...
                Console.WriteLine(loop + ":" + ret.Value + ":" + s.Value + ":" + ":" + s.DistanceTo(p.Optimum).ToString("D4") + ":" + ":" + sizeOfSubset.ToString("D4") + ":" + dm100.DiameterValue + ":" + targetAct.ToString("F2") + ":" + r10.ToString("F2") + ":" + endCount);
        ...
        public ReactiveRandomLocalSearch4(long limit, int sizeOfSubset, int targetRange)
        {
            this.limit = limit;
