...
namespace CombinatorialOptimization
{
    ...
    class ReactiveRandomLocalSearch10 : IOptimizationAlgorithm
    {
        ...
        public ISolution Solve(IOptimizationProblem p, ISolution sol, DataStoringWriter w)
        {
            ...
            List<ISolution> bests = new List<ISolution>();
            ...
            for (int loop = 0; true; loop++)
            {
                ...
                //Diameter dmBest = Diameter.CalculateDiameter(bests);

                Console.WriteLine(loop + ":" + ret.Value + ":" + s.Value + ":" + dom.Value + ":" + bestDomValue + ":" + s.DistanceTo(p.Optimum).ToString("D4") + ":" + s.DistanceTo(ret).ToString("D4") + ":" + sizeOfSubset.ToString("D4") + ":" + ":" + targetAct.ToString("F2") + ":" + r10.ToString("F2") + ":" + targetRange + ":" + endCount);

                if (3 * targetRange + 1000 < loop - lastBest)
                {
                    targetHighAct = Math.Min(targetHighAct + 0.05, 0.95);
                    lastBest = loop;
                }
        ...
        public ReactiveRandomLocalSearch10(long limit, double targetHighAct, double targetLowAct, int targetRange)
        {
            this.limit = limit;
