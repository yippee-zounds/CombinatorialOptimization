...
{
    class HierarchicalComposeSearch0207 : IOptimizationAlgorithm
    {
        private long limit;
        private int ctrj;
        private int dtrj;
        ...
        public ISolution Solve(IOptimizationProblem p, ISolution sol, DataStoringWriter w)
        {
            ...
            int highSetSize = initSize * 2;
            int midSetSize = initSize * 1;
            int lowSetSize = initSize * 1;
            ...
            for (int loop = 0; true; loop++, bestLoop++)
            {
                ...
                //冗長な要素の削除
                w.WriteLine("    " + targetHighAct);
                Shrink(highSs, highSetSize, w);

                w.WriteLine("    " + targetMidAct);
                Shrink(midSs, midSetSize, w);

                w.WriteLine("    " + targetLowAct);
                Shrink(lowSs, lowSetSize, w);
        ...
        public void Shrink(List<HierarchicalRandomLocalSearchElement> ss, int targetSize, DataStoringWriter w)
        {
            while(targetSize < ss.Count())
            {
                var e = ss.ArgMax((n) => n.best.Value);
                w.WriteLine("         " + e.best.Value + " removed");
                ss.Remove(e);
            }

            foreach(var e in ss)
            {
                w.WriteLine("         " + e.best.Value);
            }
        }
        ...
        public HierarchicalComposeSearch0207(long limit, int ctrj, int dtrj, double targetHighAct, double targetMidAct, double targetLowAct, int range)
        {
            this.limit = limit;
            this.ctrj = ctrj;
            this.dtrj = dtrj;
