...
namespace CombinatorialOptimization
{
    ...
    class ReactiveRandomLocalSearch68 : IOptimizationAlgorithm
    {
        ...
        {
            List<Tuple<ReactiveRandomLocalSearch68, ISolution, ISolution>> ts = new List<Tuple<ReactiveRandomLocalSearch68, ISolution, ISolution>>();
            ts.Add(new Tuple<ReactiveRandomLocalSearch68, ISolution, ISolution>(new ReactiveRandomLocalSearch68(0, targetHighAct, targetLowAct, targetRange), sol, sol));
            ISolution ret = sol;
            long endCount = this.limit;
            int subLoop = 1000;
            ...
            for (int loop = 0; ; loop++)
            {
                int retValue = ret.Value;
                
                for(int i = 0; i < ts.Count; i++)
                {
                    Console.Write(i + ":");

                    ReactiveRandomLocalSearch68 rrls = ts.ElementAt(i).Item1;
                    ISolution s = ts.ElementAt(i).Item2;
                    ISolution pBest = ts.ElementAt(i).Item3;

                    ISolution next = rrls.SolveStep(p, s, w, ref endCount, ret, ref loop);
                    if (next.Value < pBest.Value) pBest = next.Clone();

                    ts.RemoveAt(i);
                    ts.Insert(i, new Tuple<ReactiveRandomLocalSearch68, ISolution, ISolution>(rrls, next, pBest));

                    if (next.Value < ret.Value) ret = next.Clone();
                    if (endCount < 0) return ret;

                    Diameter dm = Diameter.CalculateDiameter(ts.Select((x) => x.Item2).ToList());
                    Console.WriteLine(":" + dm.DiameterValue + ":" + dm.Inmeter);
                }

                if (subLoop < loop && ret.Value < retValue)
                {
                    ts.Add(new Tuple<ReactiveRandomLocalSearch68, ISolution, ISolution>(new ReactiveRandomLocalSearch68(0, targetHighAct, targetLowAct, targetRange), ret.Clone(), ret.Clone()));
                    if (10 < ts.Count())
                    {
                        int worst = ts.Max((t) => t.Item3.Value);

                        for (int j = 0; j < ts.Count(); j++)
                        {
                            if (ts.ElementAt(j).Item3.Value == worst)
                            {
                                ts.RemoveAt(j);
                                break;
                            }
        ...
        public ISolution SolveStep(IOptimizationProblem p, ISolution sol, DataStoringWriter w, ref long endCount, ISolution ret, ref int loop)
        {
            ISolution s = sol.Clone();
                ...
                Console.Write(loop + ":" + ret.Value + ":" + s.Value + ":" + dom.Value + ":" + s.DistanceTo(p.Optimum).ToString("D4") + ":" + s.DistanceTo(ret).ToString("D4") + ":" + sizeOfSubset.ToString("D4") + ":" + Diameter.CalculateDiameter(ss).DiameterValue + ":" + targetAct.ToString("F2") + ":" + currentAct.ToString("F2") + ":" + endCount);
        ...
        public ReactiveRandomLocalSearch68(long limit, double targetHighAct, double targetLowAct, int targetRange)
        {
            this.limit = limit;
            ...
            sizeOfSubset = this.initialSizeOfSubset;
