...
namespace CombinatorialOptimization
{
    ...
    class ReactiveRandomLocalSearch65 : IOptimizationAlgorithm
    {
        ...
        public ISolution Solve(IOptimizationProblem p, ISolution sol, DataStoringWriter w)
        {
            ...
            for (int loop = 0; true; loop++)
            {
                ...
                ISolution tmp = null;
                if (1000 < bestLoop && loop % 2 == 0)
                {
                    tmp = moveByDistance(p, s, ret, 20);
                }
                else{
                    int nrange = 3;
                    ...
                    tmp = moveByActivity(p, s, currentAct, targetAct, ref sizeOfSubset);
                }
        ...
        private ISolution moveByDistance(IOptimizationProblem p, ISolution s, ISolution o, int dist)
        {
            int sizeOfNeighborhood = p.OperationSet().Count();
            ISolution[] origin = new ISolution[]{o};
            IEnumerable<IOperation> ops = p.OperationSet().RandomSubset(sizeOfNeighborhood, sizeOfNeighborhood);
            int min = ops.Min((op) => Math.Abs(dist - s.OperationDistance(op, origin)));
            IOperation tmpOp = ops.Where((op) => Math.Abs(dist - s.OperationDistance(op, origin)) == min).ArgMinStrict((op) => p.OperationValue(op, s));
            //tmpOp = ops.Where((op) => s.DistanceTo(o) <= s.OperationDistance(op, origin)).ArgMinStrict((op) => p.OperationValue(op, s));

            int delta = p.OperationValue(tmpOp, s);
            return s.Apply(tmpOp);
        }
        ...
        public ReactiveRandomLocalSearch65(long limit, double targetHighAct, double targetLowAct, int targetRange)
        {
            this.limit = limit;
