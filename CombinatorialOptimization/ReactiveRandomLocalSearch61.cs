...
namespace CombinatorialOptimization
{
    ...
    class ReactiveRandomLocalSearch61 : IOptimizationAlgorithm
    {
        ...
        public ISolution Solve(IOptimizationProblem p, ISolution sol, DataStoringWriter w)
        {
            ...
            for (int loop = 0; true; loop++)
            {
                ...
                if (targetAct == targetLowAct && ss.ElementAt(ss.Count() - 2).DistanceTo(s) == 0 && 0 <= lLoop)
                {
                    targetLowAct += 0.01;
                    lLoop = -100;
                }

                if (targetAct == targetHighAct && ret.Value * 1.05 < s.Value && 0 <= hLoop)
                {
                    targetHighAct -= 0.01;
                    hLoop = -100;
                }
                ...
                if (0 <= subLoop)
                {
                    ...
                    if (Double.IsNaN(r10) || r10 < targetAct)
                    {
        ...
        public ReactiveRandomLocalSearch61(long limit, double targetHighAct, double targetLowAct, int targetRange)
        {
            this.limit = limit;
