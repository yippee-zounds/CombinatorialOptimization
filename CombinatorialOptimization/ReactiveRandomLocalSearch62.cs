...
namespace CombinatorialOptimization
{
    ...
    class ReactiveRandomLocalSearch62 : IOptimizationAlgorithm
    {
        ...
        public ISolution Solve(IOptimizationProblem p, ISolution sol, DataStoringWriter w)
        {
            ...
            for (int loop = 0; true; loop++)
            {
                ...
                double currentAct = (double)s.DistanceTo(s10) / d10;

                Console.WriteLine(loop + ":" + ret.Value + ":" + s.Value + ":" + dom.Value + ":" + s.DistanceTo(p.Optimum).ToString("D4") + ":" + s.DistanceTo(ret).ToString("D4") + ":" + sizeOfSubset.ToString("D4") + ":" + ":" + targetAct.ToString("F2") + ":" + currentAct.ToString("F2") + ":" + endCount);
                if ((loop + 1) % 1000 == 0)
                {
                ...
                if (actLoop <= loop)
                {
                    if (ss.ElementAt(ss.Count() - 2).DistanceTo(s) == 0)
                    {
                        targetAct = targetHighAct;
                        actLoop = loop + targetRange;
                    }
                    else
                    {
        ...
        public ReactiveRandomLocalSearch62(long limit, double targetHighAct, double targetLowAct, int targetRange)
        {
            this.limit = limit;
