...
namespace CombinatorialOptimization
{
    ...
    class ReactiveRandomLocalSearch6C : IOptimizationAlgorithm
    {
        ...
        public ISolution Solve(IOptimizationProblem p, ISolution sol, DataStoringWriter w)
        {
            ...
            w.WriteLine("loop:best:r:vx:doptx:vdom:ddomx:doptdom:r10:lta:tact:dm:dm100:dom:x");
            ...
            for (int loop = 0; true; loop++)
            {
                ...
                Console.WriteLine(loop + ":" + ret.Value + ":" + s.Value + ":" + dom.Value + ":" + highBest.Value + ":" + s.DistanceTo(p.Optimum).ToString("D4") + ":" + ":" + sizeOfSubset.ToString("D4") + ":" + r10_2.ToString("F2") + ":" + targetAct.ToString("F2") + ":" + r10.ToString("F2") + ":" + endCount + add);
                ...
                ++bestLoop2;
                ++highLoop;
                w.WriteLine(Trace(loop, ret.Value, 0, p.Optimum, dom, r10, lta, (dm == null ? 0 : dm.DiameterValue), dm100.DiameterValue, targetAct, s));
                ...
                if (s.Value < ret.Value)
                {
                    ...
                    if (10 <= rets.Count())
                    {
                        dm = Diameter.CalculateDiameter(rets.Skip(rets.Count() - 10));
                    ...
                    if (10 <= rets.Count())
                    {
                        ...
                        lta = (double)rets[rets.Count() - 1].DistanceTo(rets[Math.Max(rets.Count() - dd, 0)]) / ltd;
                    ...
                    /*
                    if (rets.Count() == 2) Console.Write("best={" + rets[0].DistanceTo(rets[1]));
                    else if (3 <= rets.Count()) Console.Write("," + rets[rets.Count() - 2].DistanceTo(rets[rets.Count() - 1]));
                    /*
                    if (2000 < loop)
                    {
                        rets.Add(ret.Clone());
                        for (int i = 0; i < rets.Count(); i++)
                        {
                            ...
                            {
                                Console.Write(rets[i].DistanceTo(rets[j]).ToString("D3") + " ");
                            }
                            Console.Write("\n");
                        }
                        Console.Write("\n");
        ...
        private string Trace(int loop, int sizeOfSubset, double ratio, ISolution opt, ISolution dom, double r10, double lta, double dm10, double dm100, double targetAct, ISolution x)
        {
            return loop + ":" + sizeOfSubset + ":" + ratio.ToString("F3") + ":" + x.Value + ":" + opt.DistanceTo(x) + ":" + dom.Value + ":" + dom.DistanceTo(x) + ":" + dom.DistanceTo(opt) + ":" + r10.ToString("F3") + ":" + lta.ToString("F3") + ":" + targetAct.ToString("F3") + ":" + dm10 + ":" + dm100 + ":" + dom + ":" + x;
        }
        ...
        public ReactiveRandomLocalSearch6C(long limit, double targetHighAct, double targetLowAct, int targetRange)
        {
            this.limit = limit;
