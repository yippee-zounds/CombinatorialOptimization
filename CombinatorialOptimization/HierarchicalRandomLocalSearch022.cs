...
{
    class HierarchicalRandomLocalSearch022 : IOptimizationAlgorithm
    {
        private long limit;
        private int trj;
        ...
        public ISolution Solve(IOptimizationProblem p, ISolution sol, DataStoringWriter w)
        {
            ...
            for (int loop = 1; true; loop++, bestLoop++, subLoop1++, subLoop2++)
            {
                ...
                {
                    if (0 < Mid(root).Where((n) => All(n).Min((m) => m.best.Value) != All(root).Min((m) => m.best.Value)).Count())
                    {
                        Remove(root, Mid(root).Where((n) => All(n).Min((m) => m.best.Value) != All(root).Min((m) => m.best.Value)).ArgMax((n) => n.bestLoop));
                    }
                    else
                    {
                        Remove(root, Mid(root).ArgMax((n) => n.bestLoop));
                    }
                }
        ...
        */
        ...
        public HierarchicalRandomLocalSearch022(long limit, int trj, double targetHighAct, double targetMidAct, double targetLowAct, int range)
        {
            this.limit = limit;
            this.trj = trj;
